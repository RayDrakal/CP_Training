...
public:
	...
	ll mupdate(int node, ll l, ll r, ll tg, ll val) {
		if (tg<l || tg>r) return seg[node];
		if (tg == l && tg == r) return seg[node] = val;
		ll mid = (l + r) / 2;
		return seg[node] = max(mupdate(node * 2, l, mid, tg, val),
			mupdate(node * 2 + 1, mid + 1, r, tg, val));
	}
	ll qer(int node, ll l, ll r, ll xl, ll xr) {
		if (l > xr || r < xl) return 1e6 + 1;
		if (xl <= l && r <= xr) return seg[node];
		ll mid = (l + r) / 2;
		return min(qer(node * 2, l, mid, xl, xr),
			qer(node * 2 + 1, mid + 1, r, xl, xr));
	}
	ll mqer(int node, ll l, ll r, ll xl, ll xr) {
		if (l > xr || r < xl) return 0;
		if (xl <= l && r <= xr) return seg[node];
		ll mid = (l + r) / 2;
		return max(mqer(node * 2, l, mid, xl, xr),
			mqer(node * 2 + 1, mid + 1, r, xl, xr));
	}
};
int main() {
	FASTIO;
	...
	while (k--) {
		...
		cin >> a >> b;
		a--, b--;
		cout << S.mqer(1, 0, n - 1, a, b) - s.qer(1, 0, n - 1, a, b) << '\n';
	}
}